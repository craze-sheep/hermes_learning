...
### Researcher: Literature Survey B2B Checklist

When assigned a literature survey task (e.g., "查阅不少于30篇论文"):

1. **Read the project analysis first** — the Developer's report (e.g., `project-analysis.md`) defines the research questions. Understand the model architecture, inputs, and what needs to be replaced.
2. **Compile paper list from training knowledge** — group papers by research direction (typically 3-5 directions). Include title, authors, year, venue, arXiv link for each.
3. **Verify critical papers via arXiv title search** — use `arxiv.org/search/?query=%22Exact+Title%22&searchtype=all` for the 5-10 most important papers. NEVER guess arXiv IDs from memory (80% miss rate, wastes MCP fetch calls).
4. **Write the report to the task's files directory** — e.g., `artifacts/tasks/<task_id>/files/literature-review.md`. Include: paper list table, per-direction analysis, feasibility conclusion.
5. **Mark verified vs unverified citations** — papers verified via arXiv get ✅, knowledge-based papers get [待验证]. This is critical for credibility.
6. **Include a feasibility conclusion** — the Supervisor needs a clear answer: can the model be adapted? What's the recommended approach? What are the risks?

**Pitfall: MCP fetch rate limiting during bulk research.** After ~12 rapid fetch calls, the MCP server enters 429 cooldown (~60s). Prevention: alternate between fetch and browser navigation, batch calls with gaps. See `literature-survey` skill for detailed fallback strategies.

**Pitfall: ArXiv ID guessing.** Guessing IDs from partial memory produces ~80% wrong results. Each wrong guess wastes a fetch call + analysis time. Always search by exact title on arxiv.org/search when the ID is uncertain.

**Common output structure:**
```
MESSAGE: summary of findings + key conclusion + file reference
FILE: artifacts/tasks/<task_id>/files/literature-review.md (full survey with tables, per-direction analysis, conclusion)
HANDOFF_SUMMARY: <=300 chars, papers count + key conclusion + recommendation
```
