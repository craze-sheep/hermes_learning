...
- **PITFALL: VSCode plugin ≠ CLI** — If the user says they "have Claude Code" but `which claude` returns nothing, they likely have the **VSCode extension** only. The CLI is a separate install. Always check with `which claude` before assuming it's available.
...
17. **VS Code extension "Subprocess initialization did not complete within 60000ms"** — This is usually NOT an auth or network issue despite the error message. In WSL2, the most common root cause is **inotify watch exhaustion** (`fs.inotify.max_user_watches` at 100%). The extension log at `~/.vscode-server/data/logs/<session>/exthost*/Anthropic.claude-code/Claude VSCode.log` will show `ENOSPC: no space left on device, watch '...'`. Fix: `sudo sysctl fs.inotify.max_user_watches=2097152`. See `wsl2-system-audit` skill for full diagnostic path.
