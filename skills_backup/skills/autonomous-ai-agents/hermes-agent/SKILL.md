...
**Recommended: Start with tmux for persistence (especially on WSL):**
```bash
tmux new-session -d -s hermes 'hermes gateway run'
```
This keeps the gateway running after terminal closes. Manage with:
- `tmux ls` — list sessions
- `tmux attach -t hermes` — attach to session
...
**Weixin setup guide:** See `references/weixin-setup.md` for detailed Weixin/WeChat configuration, troubleshooting, and common issues.

**Local gateway exposure:** See `references/local-gateway-exposure.md` for exposing local Hermes gateway to mobile clients (OpenClaw/龙虾) via cpolar/ngrok. Covers token location, cpolar setup, and mobile client configuration.
...
**Gateway and CLI sessions are independent.** Messages from messaging platforms (Weixin, Telegram, etc.) go to gateway sessions, not the CLI session. View gateway sessions with:
```bash
