...
- **Holographic memory backup & maintenance:** See `references/holographic-memory-backup-maintenance.md` for the backup script, sqlite3 dependency pitfall (memory export silently skipped without it), Hermes cron vs system crontab scheduling, and monthly cleanup setup.
- **CodeGraph code intelligence server:** See `references/codegraph-usage.md` for init/index workflow, MCP tools reference, CLI commands, status output interpretation, and pitfalls.
