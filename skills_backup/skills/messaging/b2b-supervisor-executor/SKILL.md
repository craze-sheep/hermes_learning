...
### 8. Check messaging channel availability BEFORE dispatching (not after)

When you receive a B2B supervisor task, your first decision is: **can I actually reach worker bots?**

**Check this BEFORE writing any dispatch:**
- Use `send_message(action='list')` to see available messaging targets
- If Telegram is not in the list (e.g., only Weixin, or nothing), you CANNOT dispatch to Telegram worker bots

**Why this matters:** Writing B2B response markers and dispatching to @bot_username is wasted effort if there's no Telegram channel. You'll output the markers, then realize you need to execute directly anyway — wasting a full turn.

**Pattern:**
1. Read job JSON → understand task
2. `send_message(action='list')` → check for Telegram
3. If Telegram available → dispatch to workers normally
4. If Telegram NOT available → execute directly, skip all dispatch logic

**Anti-pattern (what happened):** Read job → dispatch to @bot → output B2B markers → user points out no Telegram → redo everything yourself. This wastes turns and confuses the user.
