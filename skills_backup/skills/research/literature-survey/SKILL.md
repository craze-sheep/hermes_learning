...
12. **NEVER guess arXiv IDs from partial memory** — This is the #1 time sink in literature surveys. When you only vaguely remember a paper's arXiv ID, guessing produces ~80% wrong results (each wrong guess wastes a fetch call + analysis time). **Correct strategy for unknown IDs:** (a) search by exact title on `arxiv.org/search/?query=%22Exact+Paper+Title%22&searchtype=all`, (b) navigate to the search result page in browser, (c) extract the correct arXiv ID from the result. This 3-step title-search approach is slower per paper but has ~100% accuracy, vs ~20% for guessing. Tested in session 2026-06-02: guessed 15 IDs, only 3 correct; searched by title for 6 papers, all 6 correct.

13. **MCP fetch rate limiting pattern** — The MCP fetch server rate-limits after ~10-12 rapid consecutive calls (HTTP 429), then becomes unreachable for ~60s. **Prevention:** (a) batch your fetch calls with 2-3s gaps between groups, (b) prefer browser navigation for individual arxiv pages (browser is NOT rate-limited by the MCP server), (c) when doing bulk paper verification, alternate between fetch (for API calls) and browser (for arxiv abs pages). **Recovery:** if fetch goes down, switch entirely to browser-based verification until the cooldown expires. Do NOT retry fetch immediately — the error message says "Auto-retry available in ~Ns" — wait that long.

14. **Bulk paper survey B2B workflow** — When the B2B task requires "不少于30篇论文" (30+ papers), the most efficient workflow is: (a) compile paper list from training knowledge FIRST (grouped by research direction), (b) use browser to arxiv.org/search for exact-title verification of the most critical papers (5-10), (c) write the report with verified papers marked ✅ and knowledge-based papers marked [待验证], (d) do NOT try to verify all 30+ papers one by one — it takes too long and the MCP fetch will rate-limit. The report should clearly distinguish verified vs unverified citations.
