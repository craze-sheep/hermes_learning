...
Use multiple AI agents (Hermes + Claude Code + Codex + OpenCode) to independently verify code, then cross-compare findings for higher confidence.

## User's Required Review Order

```
1. Hermes + OpenCode (same model, mimo2.5) iterate on task
   - Design docs, implementation, first-pass reviews
   - Fast iteration, same perspective = good for iteration
   
2. Claude Code (DIFFERENT model) does FINAL review only
   - Catches blind spots from the two mimo2.5 agents
   - ONE final review per task (not iterative back-and-forth)
   - If fails: Hermes+OpenCode fix → Claude Code re-reviews once
   
3. Mark task complete ONLY after Claude Code approves
```

**Critical**: User explicitly corrected skipping Claude Code review. Never mark a task complete without Claude Code final review.
