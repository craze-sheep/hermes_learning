...
When a user says something exists that you don't see in docs:
1. **Check the filesystem first** — `search_files` for the feature name, adapter, plugin, or module
2. **Check config files** — look for platform keys, plugin entries, environment variables
3. **Check logs** — search for connection attempts, error messages, or initialization traces
4. **Only then** state whether it's configured vs. missing

**Pattern from real session:** User asked about QQ Bot support. Hermes-agent skill listed supported platforms but QQ wasn't explicitly mentioned. I asserted "QQ isn't supported." User corrected me. Filesystem check revealed `gateway/platforms/qqbot/` existed with full adapter code, `config.yaml` had qqbot entries, and `channel_directory.json` had qqbot registered. The real issue was missing credentials, not missing platform support.

**The correct framing is always:** "Let me verify" → check filesystem → report what's actually there (or not). Never: "Docs don't mention it, so it doesn't exist."
